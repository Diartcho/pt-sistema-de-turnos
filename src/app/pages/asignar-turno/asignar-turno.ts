...
export class AsignarTurno implements OnInit {
  ...
  // opciones cada 15 min con AM/PM
  timeOptions: string[] = [];
  ...
  ngOnInit(): void {
    ...
    this.employeesService.getAll().subscribe({
      next: (emps: Employee[]) => {
        this.employee = emps.find(e => e.id === id) ?? null;

        // si no existe o está inactivo → vuelve
        if (!this.employee || this.employee.status === 'Inactivo') {
  ...
  private parseTimeToMinutes12h(value: string): number | null {
    // formato esperado: "h:mm AM" o "hh:mm PM"
  ...
  validateTimes(): boolean {
    ...
    if (start == null) {
      this.timeError = 'Hora inicio inválida. Usa formato como: 8:15 AM (minutos: 00, 15, 30, 45).';
      return false;
    }
    if (end == null) {
      this.timeError = 'Hora fin inválida. Usa formato como: 10:30 PM (minutos: 00, 15, 30, 45).';
      return false;
    }
  ...
  save(): void {
    ...
    setTimeout(() => this.router.navigate(['/empleados']), 650);
  }
